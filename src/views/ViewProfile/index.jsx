...
import { fieldToFeedback, fieldInputToValue, isValidDriverObject, requiredFields, additionalFields } from "../../Utilities/Fields";
...
class Profile extends Component {
    ...
    logoutDriver = async () => {
        ...
        window.location.href = "https://dev-2nvx8dom.auth0.com/v2/logout?returnTo=https%3A%2F%2Fportlmedia.com";
    }
    ...
    createModal = () => {
        return (
            ...
            >
                <button onClick={() => this.setModal(false)} className="modalCloseButton">
                    &times;
                </button>
                <h1 className="alertModalTitle">{this.state.modal.title}</h1>
                {this.state.modal.text && this.state.modal.text.map((msg, i) => {
                    return <p key={i} className="alertModalText">{msg}</p>
                })}
                {this.state.modal.buttonPrompt &&
                    <Button onClick={this.state.modal.onClick} className="optionalModalButton">
                        {this.state.modal.buttonPrompt}
                    </Button>
                }
            </Modal>
    ...
    createNotification = () => {
        ...
        if (this.state.notification.isError) {
            ...
            if (!msg) {
                msg = "Something went wrong, try again in a few minutes or contact us for further support"
            }
        } else {
        ...
        return (
            ...
            >
                <button onClick={() => this.setNotification(false)} className="notificationCloseButton">
                    &times;
                </button>
    ...
    receiveDriver = async (response) => {
        ...
        if (response.status === 200) {
            ...
            this.handleError("You are not signed in. Please sign in to visit this page.")
        }
    }
    ...
    handleFormSubmission = async (driverUpdate, picture, ref) => {
        ...
        if (this.state.isSignUp && !isValidDriverObject(driver)) {
            ...
            requiredFields.concat(additionalFields).forEach(field => {
                if (field === "email" || typeof driver[field] !== "string") { return; }
        ...
        if (invalidRequiredFields.length === 0) {
            this.setState({ driver: driver }, () => { this.refreshDriverInfo(driver, picture, ref) })
            return true
        } else {
    ...
    refreshDriverInfo = async (driver, picture, ref) => {
        ...
        if (this.state.isSignUp) {
            ...
            updateDriver(driver, profile, this.handleUpdateResponse, this.notifyError)
        }
    }
    ...
    handleCreateResponse = async (res) => {
        ...
        if (res.status === 200) {
            this.setNotification(true, "Success!", "Your Account Was Created!", false)
        } else {
    ...
    handleUpdateResponse = async (res) => {
        ...
        if (res.status === 200) {
            this.setNotification(true, "Success", "Your changes have been saved!", false)
        } else if (res.status === 400) {
            this.setNotification(true, "Error", "There was an error with authenticating your account", true)
            this.setState({ isLoading: false })
